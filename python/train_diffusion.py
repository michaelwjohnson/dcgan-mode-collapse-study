...
def denoise_images_experiment(
    ...
    # Load diffusion model first to check its expected input size
    print("Loading diffusion model...")
    try:
        # Load pre-trained DDPM model trained on MNIST
        model = DDPMPipeline.from_pretrained("1aurent/ddpm-mnist").to(device)
        scheduler = model.scheduler
        unet = model.unet

        # The model expects 28x28 grayscale images (MNIST native resolution)
        img_size = 28
        print(
            f"Model loaded successfully. Expected input: {img_size}x{img_size} grayscale"
        )
    ...
    # Load MNIST test images with the correct size for the model
    ...
    for noise_std in noise_levels:
        ...
        with torch.no_grad():
            for t in tqdm(scheduler.timesteps, desc=f"Denoising (noise={noise_std})"):
                ...
                denoised_images = scheduler.step(
                    model_output, t, denoised_images
                ).prev_sample
        ...
        plt.suptitle(
            f"Denoising Diffusion Process (noise std = {noise_std})", fontsize=14
        )
        plt.tight_layout()
        plt.savefig(
            f"{output_dir}/denoise_noise_{noise_std}.png", dpi=150, bbox_inches="tight"
        )
        plt.close()

        print(
            f"  Time: {denoise_time:.2f}s ({denoise_time/num_samples*1000:.2f}ms per sample)"
        )
...
def compare_with_gan(
    diffusion_dir="output/diffusion_denoise",
    ...
    # Load metrics
    comparison = {"diffusion": {}, "gan_baseline": {}, "gan_modified": {}}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Diffusion Model Denoising Experiments"
    )
    parser.add_argument(
        "--mode",
        type=str,
