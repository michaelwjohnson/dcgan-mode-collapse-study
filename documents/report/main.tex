...
\begin{abstract}
This paper presents a comparative study of Deep Convolutional Generative Adversarial Networks (DCGANs) and diffusion models applied to MNIST digit generation and denoising. The study investigates the effects of different architectural configurations on training stability and output quality by comparing a baseline DCGAN with a modified architecture featuring altered latent dimensions, network depth, and regularization. Experiments demonstrate how hyperparameter choices significantly impact GAN performance, with the baseline configuration producing diverse, high-quality samples while the modified architecture exhibits severe mode collapse. Comparative analysis reveals fundamental tradeoffs: GANs offer fast inference (1.29ms/sample) but may suffer from training instability and mode collapse, while diffusion models provide inherent stability and consistent quality at the cost of 115$\times$--230$\times$ slower inference (153--297ms/sample for 100 denoising steps). These findings validate theoretical predictions from lecture material and provide practical insights for model selection and scaling considerations in production environments.
\end{abstract}
...
Generative Adversarial Networks (GANs) \cite{goodfellow2014generative} consist of a generator and discriminator competing in a minimax game. DCGANs \cite{radford2015unsupervised} use convolutional architectures for improved image synthesis. Diffusion models iteratively denoise random noise to generate data and demonstrate stable training with high sample quality.
...
\begin{enumerate}
    \item \textbf{Reduced Latent Dimension (64 vs. 256):} Constrains the generator's expressive capacity, limiting representation of diverse digit modes. Insufficient capacity prevents learning the full distribution.
...
GANs represent minimax games seeking Nash equilibrium:
\[
\min_G \max_D \mathbb{E}_{x \sim p_{data}}[\log D(x)] + \mathbb{E}_{z \sim p_z}[\log(1 - D(G(z)))]
\]

Ideally, equilibrium occurs when $D$ cannot distinguish real from fake ($D(x) = D(G(z)) = 0.5$) and $G$ has learned $p_{data}$. This equilibrium is often unstable. The modified model demonstrates failure to reach equilibrium:
\begin{itemize}
...
When $D$ becomes too powerful relative to $G$, gradients for $G$ may vanish or become uninformative. The modified model exemplifies this:
\begin{itemize}
...
When $D$ perfectly classifies fake samples, $\log(1 - D(G(z))) \approx 0$, causing gradients for $G$ to vanish. The modified model's discriminator loss of $\sim$0.0001 confirms near-perfect classification, explaining why the generator cannot escape collapse despite continued training.

\textbf{4. Hyperparameter Sensitivity:}

GAN training is highly sensitive to hyperparameter choices. The baseline and modified models differ only in latent dimension, depth, and dropout, yet produce substantially different outcomes:
\begin{itemize}
    \item Baseline: Stable training, diverse outputs
    \item Modified: Complete collapse, meaningless outputs
\end{itemize}

This sensitivity demonstrates the importance of careful architecture design and hyperparameter tuning.

\textbf{5. Lack of Convergence Guarantees:}

GANs lack theoretical convergence guarantees, unlike maximum likelihood methods. The modified model's inability to recover from collapse after epoch 30 (despite continued training through epoch 50) illustrates this limitation. Once collapse occurs, the adversarial dynamics do not naturally restore equilibrium.

\textbf{Connection to Proposed Solutions:}

Techniques to mitigate mode collapse and instability include:
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Reconstruction vs. Generation:} At $\sigma = 0.3$ (low noise), denoised outputs preserve more similarity to originals but still exhibit generative variation. At higher noise levels ($\sigma = 0.5, 0.7$), the model generates digits more freely from the learned distribution. This behavior contrasts with deterministic denoisers (e.g., autoencoders) which attempt exact reconstruction.
\end{itemize}

\textbf{Key Observations:} The DCGAN baseline generates diverse, sharp digits from random latent vectors. The diffusion model denoises noisy MNIST images, producing realistic handwritten digits. The denoised outputs do not exactly match the original inputs, revealing a fundamental characteristic: diffusion models are \textbf{generative} rather than reconstructive. The model samples from the learned MNIST distribution conditioned on the noisy input, potentially producing different digit classes or styles than the originals. This contrasts with deterministic denoising methods (e.g., denoising autoencoders) which aim for pixel-wise reconstruction. For applications requiring exact reconstruction, this generative behavior presents a limitation; for applications requiring diverse, realistic outputs, it offers an advantage.
...
\textit{Inference Speed:} The DCGAN achieves 115$\times$--230$\times$ faster inference (1.29ms vs. 153--297ms per sample) due to single-pass generation. Diffusion models require 100 sequential denoising steps, making them computationally expensive at inference time. The variation in diffusion timing (153--155ms for $\sigma=0.5,0.7$ vs. 297ms for $\sigma=0.3$) likely reflects GPU warmup effects and batch processing overhead, with the first denoising pass showing higher latency due to model initialization.
...
These metrics, consistent across noise levels, provide quantitative evidence that diffusion models generate samples from the learned distribution rather than reconstructing inputs. This behavior contrasts with deterministic denoisers (e.g., denoising autoencoders) which typically show PSNR $>$30 dB and correlation $>$0.9.

\textit{Metric Limitations:} Formal metrics like Inception Score (IS) or Fréchet Inception Distance (FID) require pretrained classifiers unavailable in this environment due to filesystem constraints. The denoising task differs from unconditional generation, complicating direct quality comparison. Diversity metrics (inter-sample variance, pairwise distance) and reconstruction metrics (MSE, PSNR, correlation) provide objective quantitative comparison.
